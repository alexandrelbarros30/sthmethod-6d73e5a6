...
BEGIN
  ...
  ELSIF NEW.kind = 'analysis' THEN
    INSERT INTO public.student_clinical_analyses (id, user_id, title, scope, report_html, released_to_student, released_at, created_at, updated_at)
    VALUES (NEW.id, NEW.user_id, 'Análise STH AI', 'full', NEW.content, true, NEW.created_at, NEW.created_at, now())
