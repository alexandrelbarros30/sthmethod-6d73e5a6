...
CREATE POLICY "Admins can manage message categories" ON public.message_categories FOR ALL USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Anyone authenticated can view categories" ON public.message_categories FOR SELECT USING (true);
...
CREATE POLICY "Admins can manage message templates" ON public.message_templates FOR ALL USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
...
CREATE POLICY "Admins can manage message history" ON public.message_history FOR ALL USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
