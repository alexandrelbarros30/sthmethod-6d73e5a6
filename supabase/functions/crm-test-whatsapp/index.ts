...
// Fallback: secrets de ambiente (ZAPI_*, WAPI_*) para compatibilidade.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (provider === 'wapi') {
      ...
      if (!id || !tok) {
        return new Response(JSON.stringify({ ok: false, error: 'Credenciais W-API ausentes. Preencha Instance ID e Token e clique em Salvar.', missing: { instance_id: !id, token: !tok } }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
