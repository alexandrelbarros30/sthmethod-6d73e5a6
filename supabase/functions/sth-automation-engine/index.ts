...
function classifyIntent(text: string): string {
  ...
  if (
    /(pix|pagamento|boleto|cartao|cartão|cobranc|fatura|valor|preço|preco|quanto custa)/.test(
      t,
    )
  )
    return "financeiro";
  if (/(plano|consultoria|começar|comecar|contratar|inscrev)/.test(t))
    return "comercial";
  return "duvida";
}
...
function scoreFromText(text: string): { delta: number; reasons: string[] } {
  ...
  if (/(valor|preço|preco|quanto custa)/.test(t)) {
    delta += 20;
    reasons.push("Perguntou valor (+20)");
  }
  if (/(plano|consultoria)/.test(t)) {
    delta += 15;
    reasons.push("Perguntou plano (+15)");
  }
  if (/(pix|pagamento|boleto|cartao|cartão)/.test(t)) {
    delta += 20;
    reasons.push("Perguntou pagamento (+20)");
  }
  if (/(cadastro|inscrev|começar|comecar)/.test(t)) {
    delta += 30;
    reasons.push("Demonstrou intenção de cadastro (+30)");
  }
...
async function sendAutoReply(
  ...
  const normalizedProvider =
    String(provider || "").toLowerCase() === "zapi" ? "zapi" : "wapi";
  const primaryFn =
    normalizedProvider === "zapi" ? "send-whatsapp" : "send-wapi";
  const fallbackFn =
    normalizedProvider === "zapi" ? "send-wapi" : "send-whatsapp";

  const sendVia = (fnName: string) =>
  ...
  const failureBody = await response
    ...
    .text()
    .catch(() => "");
  console.error("auto reply primary send failed", {
  ...
  if (
    !failureBody ||
    /credentials missing|instance|channel|provider|unauthorized|forbidden|not found|invalid/i.test(
      failureBody,
    )
  ) {
    const fallbackResponse = await sendVia(fallbackFn);
    if (fallbackResponse.ok) {
      console.warn("auto reply sent via fallback provider", {
...
async function handleInbound(supabase: any, body: AnyRec) {
  ...
  const { data: profile } = await supabase.rpc("find_profile_by_phone", {
    _phone: phone,
  });
  ...
  if (memId && delta > 0) {
    for (const r of reasons) {
      await supabase
        .from("sth_auto_score_log")
        .insert({ memory_id: memId, phone, reason: r, delta });
    }
      ...
      if (tpl?.enabled && tpl?.message) {
        ...
        const daysLeft = sub?.end_date
          ? Math.max(
              0,
              Math.floor(
                (new Date(sub.end_date).getTime() - Date.now()) / 86400000,
              ),
            )
          : 0;
        ...
        try {
          const greetingResponse = await sendAutoReply(
            phone,
            rendered,
            replyProvider,
          );
          if (greetingResponse.ok) {
            ...
            console.error("greeting send failed", {
              phone,
              replyProvider,
              errorText,
            });
  ...
  const responseEngine = (autoReplyCfg?.engine || "local") as
    | "local"
    | "ai"
    | "gemini";
  ...
  try {
    if (!nutriRedirectSent && autoReplyEnabled && responseEngine === "local") {
      ...
      if (localResult.reply?.trim()) {
        const sendRes = await sendAutoReply(
          phone,
          localResult.reply,
          replyProvider,
        );
    ...
    if (
      ...
      autoReplyEnabled &&
      !autoReplySent &&
      responseEngine !== "local"
    ) {
      const draftResult = await createDraft();
      if (draftResult.ok && draftResult.draftId) {
        const sendRes = await fetch(
          `${SUPABASE_URL}/functions/v1/sth-ai-engine`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
function renderBillingTemplate(
  content: string,
  ctx: Record<string, any>,
): string {
  const firstName =
    (ctx.full_name as string | undefined)?.split(" ")[0] || "Aluno";
  ...
  const link = ctx.user_id
    ? `${ctx.origin}/dashboard/renew?uid=${ctx.user_id}`
    : "";
...
async function handleBillingTick(supabase: any) {
  ...
  const { data: subs } = await supabase
    .from("subscriptions")
    .select("user_id, end_date, plan_id")
    .in("user_id", userIds);
  ...
  for (const camp of campaigns) {
    ...
    try {
      ...
      if (j?.ok) autoOk = true;
      else deliveryError = j?.error || "falha no envio";
    } catch (err: any) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    else if (action === "billing_tick")
      result = await handleBillingTick(supabase);
