...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const userPrompt = mode === 'revise'
      ? `Perfil do usuário:\n${context}\n${fbContext ? `\n${fbContext}\n` : ''}${libraryBlock ? `\n${libraryBlock}\n` : ''}\nVERSÃO ANTERIOR (a ser corrigida):\n${last!.content}\n\nAJUSTE PEDIDO (CONTRA-RESPOSTA CRÍTICA): ${instruction}\n\nINSTRUÇÃO DE OBEDIÊNCIA TOTAL: O usuário está enviando uma contra-resposta específica porque a versão anterior foi ignorada ou está incorreta. Você DEVE refazer o plano incorporando EXATAMENTE as mudanças pedidas. É PROIBIDO ignorar inclusões de alimentos (ex: se pedir gelatina zero, ela DEVE aparecer). O cumprimento das solicitações de ajuste é a sua PRIORIDADE MÁXIMA. Se o usuário pedir um alimento específico, ele DEVE aparecer na BASE ou nas opções. Ajuste as porções para que os macros fechem na meta. Adicione ao final a seção "## O que mudou nesta revisão".`
      : `Perfil do usuário:\n${context}\n${fbContext ? `\n${fbContext}\n` : ''}${libraryBlock ? `\n${libraryBlock}\n` : ''}${instruction ? `\nObservações/Prompt Livre (MANDATÓRIO): ${instruction}` : ''}${exceptionReason ? `\nExceção registrada: ${exceptionReason}` : ''}`;
