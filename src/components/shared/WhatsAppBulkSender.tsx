...
import { MessageCircle, Send, ChevronDown, AlertCircle } from "lucide-react";
...
interface Props {
  /** For consultors, pass linked student IDs to filter */
...
export default function WhatsAppBulkSender({ linkedStudentIds }: Props) {
  ...
  const { data: expiringStudents = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return subs
        ...
        .filter(Boolean) as ExpiringStudent[];
    },
  });
  ...
  const selectAll = () => {
    const slice = expiringStudents.slice(0, 10);
    setSelected(new Set(slice.map((s) => s.user_id)));
  };

  const buildMessage = (student: ExpiringStudent) => {
  ...
  const sendAll = () => {
    const students = expiringStudents.filter((s) => selected.has(s.user_id));
      ...
      <Card className="border-[#25D366]/20 bg-[#25D366]/5">
        ...
        <CollapsibleContent>
          <CardContent className="space-y-4 pt-0">
            {expiringStudents.length === 0 ? (
              <p className="text-sm text-muted-foreground py-2">Nenhum aluno com plano vencendo nos próximos 3 dias.</p>
            ) : (
              <>
                {/* Template selector */}
                <div>
                ...
                <div className="flex items-center justify-between">
                  <p className="text-xs text-muted-foreground">
                    {selected.size} de {Math.min(expiringStudents.length, 10)} selecionados
                  </p>
                ...
                <ScrollArea className="max-h-[300px]">
                  <div className="space-y-1">
                    {expiringStudents.map((student) => {
                      const dayBadge = getDayBadge(student.days_left);
                      return (
                        <div
                          key={student.user_id}
