...
const AiNextMealCard = ({ diet }: { diet: AiGeneration | null }) => {
  ...
  return (
    <Link
      to="/ai/app/cardapio"
      className="group relative col-span-2 flex flex-col justify-between overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-ocean-mid to-ocean-deep p-5 text-white shadow-xl shadow-ocean-deep/20 transition-all duration-300 hover:-translate-y-0.5 lg:col-span-3 lg:p-6"
    >
      <div
      ...
      {current ? (
        <>
          <div className="relative mt-4 min-w-0">
            <p className="truncate font-urbanist text-lg font-bold leading-tight tracking-tight sm:text-xl">
              {current.name}
            </p>
            {base && (
              <p className="mt-1.5 line-clamp-2 text-[13px] leading-relaxed text-white/70">{base.text}</p>
            )}
          </div>

          <div className="relative mt-4 flex flex-wrap items-center gap-1.5">
            <span className="inline-flex items-baseline gap-1 rounded-lg border border-ocean-mint/40 bg-ocean-mint/15 px-2.5 py-1 font-mono text-[12px] font-extrabold tabular-nums text-ocean-mint">
              {current.kcal}
              <span className="text-[9px] font-semibold uppercase tracking-[0.14em] opacity-75">kcal</span>
            </span>
            <span className="inline-flex items-baseline gap-1 rounded-lg border border-sky-300/40 bg-sky-300/15 px-2.5 py-1 font-mono text-[12px] font-extrabold tabular-nums text-sky-200">
            ...
            <span className="inline-flex items-baseline gap-1 rounded-lg border border-amber-300/40 bg-amber-300/15 px-2.5 py-1 font-mono text-[12px] font-extrabold tabular-nums text-amber-200">
              <span className="text-[9px] font-bold uppercase tracking-[0.14em] opacity-80">Carb</span>
              {current.carbs}
              <span className="text-[9px] font-semibold opacity-70">g</span>
            </span>
            <span className="inline-flex items-baseline gap-1 rounded-lg border border-orange-300/40 bg-orange-300/15 px-2.5 py-1 font-mono text-[12px] font-extrabold tabular-nums text-orange-200">
