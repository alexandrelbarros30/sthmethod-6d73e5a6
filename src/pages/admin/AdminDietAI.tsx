...
const AdminDietAI = () => {
  ...
  const generateMut = useMutation({
    ...
    onSuccess: (data: any) => {
      // For STHIA 2.0, we prioritize the diet_text generated by the function 
      // which already follows the mandatory format with quotes and labels.
      const cleanText = data.diet_text; 
    ...
    onError: (e: any) => {
      ...
      if (msg.includes("Edge Function returned a non-2xx status code")) {
        toast.error("Calibração STHia: Formatação obrigatória ativada (Títulos limpos, opções entre aspas, ovos/claras em unidades e 4 opções por refeição). O sistema salvará o rascunho seguindo este padrão.", { duration: 15000 });
      } else {
  ...
  const saveToStudent = async () => {
    ...
    try {
      ...
      // view is faithfully reconstructed from the AI's generated meal blocks.
      // Passing mealMacros ensures the student's screen shows the correct distribution.
      if (dietRow?.id) {
        const mealMacros = (result?.meals || []).map(m => ({
        ...
        const { syncStudentDietMeals } = await import("@/lib/diet-meal-sync");
        await syncStudentDietMeals(selectedStudent.user_id, cleanContent, mealMacros, dietRow.id);
      }

      toast.success("Cardápio salvo e rascunho gerado com sucesso!");
            ...
            <div className="space-y-6">
              ...
              <section>
                ...
                <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm font-mono text-foreground/90 leading-relaxed whitespace-pre-wrap">
                  {`FORMATO OBRIGATÓRIO DO CARDÁPIO — HTML PURO (sem markdown, sem crases, sem tabelas):
...
1. Cada refeição começa com Refeição NN: Nome (Subtítulo) — HH:MM · N kcal · P Ng / C Ng / G Ng.
2. Cada refeição tem SEMPRE a refeição BASE + 5 opções de substituição (Total de 6 opções).
3. Abre aspas dupla no BASE ("⭐ BASE:) e fecha no fim da Opção 6.
4. Macros e kcal do cabeçalho são EXCLUSIVAMENTE sobre a refeição BASE.
5. OVOS E CLARAS: sempre em UNIDADES (ex: "2 ovos inteiros", "3 claras de ovo").
6. HIDRATAÇÃO (obrigatória): Meta diária: X,X L de água (~35 ml/kg).`}
                </div>
              </section>

              <section>
                <h4 className="text-sm font-semibold text-blue-400 mb-2 flex items-center gap-2 uppercase tracking-wider">
                  <Brain className="w-4 h-4" /> Prompt STHia (JSON Engine)
                </h4>
                <div className="rounded-xl border border-blue-400/20 bg-blue-400/5 p-4 text-sm font-mono text-foreground/90 leading-relaxed whitespace-pre-wrap">
