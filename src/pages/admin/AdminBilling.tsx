    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        ...
        {DAY_BUCKETS.map((b) => {
          ...
          return (
            <Button key={b.key} size="sm" variant={periodFilter === b.key ? "default" : "outline"} onClick={() => setPeriodFilter(b.key)}>
              {b.label} <Badge variant="outline" className="ml-2 h-5">{count}</Badge>
            </Button>
          );
        })}
      </div>
