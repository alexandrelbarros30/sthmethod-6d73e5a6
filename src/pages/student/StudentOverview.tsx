...
import {
  ...
  Home as HomeIcon, Compass, Dumbbell, TrendingUp
} from "lucide-react";
...
import BentoHomeGrid from "@/components/student/BentoHomeGrid";
...
const HydrationWidget = ({
  ...
  return (
    <div className="mb-6 rounded-3xl border border-border/40 bg-card p-6">
      ...
      <div className="flex items-end justify-between mb-4">
        <div>
          <div className="flex items-baseline gap-1">
            <span className="text-[36px] leading-none font-semibold tracking-[-0.04em] text-card-foreground tabular-nums">{consumedL}</span>
        ...
        <button
          onClick={onRemove}
          className="w-9 h-9 rounded-full border border-border/60 flex items-center justify-center text-muted-foreground hover:text-card-foreground hover:border-foreground/40 transition-colors"
      ...
      <div className="grid grid-cols-4 gap-2">
        {[200, 250, 500, 750].map((ml) => (
          <button
            key={ml}
            onClick={() => onAdd(ml)}
            className="h-10 rounded-full border border-border/60 text-[12px] font-medium text-card-foreground hover:bg-primary hover:text-primary-foreground transition-colors flex items-center justify-center gap-1"
          >
...
const StudentOverview = () => {
  ...
  return (
    <DashboardLayout role="student" title="" subtitle="">
      ...
      <div className="theme-graphite-pulse min-h-full">
        {/* HEADER */}
        <div className="flex items-start justify-between mb-8 relative">
          <div className="flex-1 min-w-0 pt-2">
            <p className="text-[11px] font-medium tracking-[0.2em] uppercase text-primary">{greeting}</p>
            <h1 className="text-[38px] sm:text-[48px] leading-[0.95] font-semibold text-foreground tracking-[-0.045em] mt-3 font-space">
              {firstName}.
            </h1>
            <p className="text-[13px] text-muted-foreground font-light mt-3 tracking-tight max-w-[280px]">
              Controle sua jornada em 4 toques. Tudo começa aqui.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex flex-col items-end mr-1">
              ...
              <span className="text-primary font-bold text-sm flex items-center gap-1">
                <Flame className="w-3.5 h-3.5" /> {streakData?.streak ?? 0} DIAS
              </span>
            </div>
            <button
              onClick={() => navigate("/dashboard/ads")}
              className="w-11 h-11 rounded-full flex items-center justify-center bg-card border border-border/40 hover:bg-surface transition-colors relative"
        ...
        {/* BENTO GRID — 4 OPÇÕES PRINCIPAIS */}
        <BentoHomeGrid
          meals={{
            completedCount,
            totalMeals,
          ...
          latestTrend={{ title: latestTrend.title, path: latestTrend.path }}
          hasTraining={hasTraining}
        />

        {/* QUICK STATS — desktop */}
        <div className="hidden md:grid grid-cols-4 gap-4 mb-8">
          <div className="rounded-2xl bg-card/60 border border-border/40 p-4 hover:border-primary/30 transition-colors">
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1 flex items-center gap-1">
              <Droplets className="w-3 h-3 text-[hsl(166,76%,64%)]" /> Hidratação
            </p>
            <p className="text-lg font-semibold text-card-foreground">
              {(dayWaterMl / 1000).toFixed(dayWaterMl >= 1000 ? 1 : 2)}L
              <span className="text-xs text-muted-foreground font-normal"> / {dayHydrationGoalL}L</span>
            </p>
          </div>
          <div className="rounded-2xl bg-card/60 border border-border/40 p-4 hover:border-primary/30 transition-colors">
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1 flex items-center gap-1">
              <Flame className="w-3 h-3 text-orange-500" /> Sequência
            </p>
            <p className="text-lg font-semibold text-card-foreground">
              {streakData?.streak ?? 0} <span className="text-xs text-muted-foreground font-normal">dias</span>
            </p>
          </div>
          <div className="rounded-2xl bg-card/60 border border-border/40 p-4 hover:border-primary/30 transition-colors">
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1 flex items-center gap-1">
              <Target className="w-3 h-3 text-primary" /> Meta kcal
            </p>
            <p className="text-lg font-semibold text-card-foreground">
              {Math.round(dayMacros?.kcal || 0)}
              <span className="text-xs text-muted-foreground font-normal"> / {Math.round(dayTargetMacros?.kcal || 0)}</span>
            </p>
          </div>
          <div className="rounded-2xl bg-card/60 border border-border/40 p-4 hover:border-primary/30 transition-colors">
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1 flex items-center gap-1">
              <Beef className="w-3 h-3 text-[hsl(166,76%,64%)]" /> Proteína
            </p>
            <p className="text-lg font-semibold text-card-foreground">
              {Math.round(dayMacros?.protein || 0)}g
              <span className="text-xs text-muted-foreground font-normal"> / {Math.round(dayTargetMacros?.protein || 0)}g</span>
            </p>
          </div>
        </div>

        {/* TABS — Jornada | Descobrir */}
                ...
                {contentSections.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      className="snap-start shrink-0 w-[55vw] max-w-[220px] text-left rounded-2xl overflow-hidden relative group border border-border/40 bg-card active:scale-[0.98] transition-all duration-300"
                    >
