      ...
      <div className="min-h-screen -m-4 sm:-m-6 px-4 sm:px-6 py-6 space-y-6 max-w-lg mx-auto" style={{ background: G.bg }}>
        ...
        <motion.header initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-3 pt-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: G.t96 }}>
            {activeFamily ? activeFamily.title : "Hormônios e Compostos"}
          </h1>
        ...
        {!activeFamily && (
          <>
            ...
            >
              ...
              >
                <h2 className="text-base font-semibold" style={{ color: G.t92 }}>3 famílias. 15 compostos.</h2>
                <ul className="space-y-1">
                  {["Testosterona — a base estrutural", "DHT — estética e definição", "19-nor — potência máxima"].map((item) => (
                    <li key={item} className="flex items-center gap-2 text-[13px]" style={{ color: G.t60 }}>
                      <ChevronRight className="w-3 h-3 flex-shrink-0" style={{ color: G.accent }} />
                      {item}
                    </li>
                  ))}
                </ul>
